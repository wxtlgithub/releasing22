    ...
    this.window = new WXTL.Widgets.CommonWindows.Window({
        ...
        loadDataStoreFunc: function(){
		    	...
		    	if (userData.data.length > 0) {
		    		var elNode = Ext.getCmp("Js.Center.Popedom.UserUpdate.numnodeid");
		    		elNode.setValue(Ext.getCmp("UserUpdateNumnodeid").getValue());
		            if (userData.data[0].numtype != -1) {
		            	if(userData.data[0].numdepartid != 1 && userData.data[0].numtype !=1){
		            		elNode.disable();
		            	}
		            }
		            else{
